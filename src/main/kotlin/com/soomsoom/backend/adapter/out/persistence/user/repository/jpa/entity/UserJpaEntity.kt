    ...
    companion object {
        fun from(user: User) : UserJpaEntity {
            return when (val account = user.account) {
                is Account.Anonymous -> UserJpaEntity(user.id ?: 0L, account.deviceId, AccountType.ANONYMOUS, null, null, null, null, user.role)
                is Account.Social -> UserJpaEntity(user.id ?: 0L, account.deviceId, AccountType.SOCIAL, account.socialProvider, account.socialId, null, null, user.role)
                is Account.IdPassword -> UserJpaEntity(user.id ?: 0L, null, AccountType.ID_PASSWORD, null, null, account.username, account.password, user.role)
            }
        }
    }
