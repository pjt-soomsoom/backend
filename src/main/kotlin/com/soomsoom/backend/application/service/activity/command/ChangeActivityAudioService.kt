...
@Service
@Transactional
class ChangeActivityAudioService(
...
) : ChangeActivityAudioUseCase {

    @PreAuthorize("hasRole('ADMIN')")
    override fun changeAudio(command: ChangeActivityAudioCommand): ChangeActivityResult {
        // id로 activity 있는지 확인
        ...
        return ChangeActivityResult(audioInfo.preSignedUrl, audioInfo.fileKey)
    }

    @PreAuthorize("hasRole('ADMIN')")
    override fun completeAudioChange(command: CompleteActivityAudioChangeCommand): ActivityResult {
        // 파일이 실제로 업로드 됐는지 확인
        ...
        val oldFileKey = activity.updateAudio(newFileUrl, command.fileKey)

        // 기존 파일 삭제
        oldFileKey?.let(fileDeleterPort::delete)

        // breathingActivity의 경우 timeline까지 업데이트
        when (command) {
            is CompleteBreathingActivityAudioChangeCommand -> {
            ...
            is CompleteMeditationActivityAudioChangeCommand -> {
