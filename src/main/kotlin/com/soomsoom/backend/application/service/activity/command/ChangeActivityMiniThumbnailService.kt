...
@Service
@Transactional
class ChangeActivityMiniThumbnailService(
...
) : ChangeActivityMiniThumbnailUseCase {

    @PreAuthorize("hasRole('ADMIN')")
    override fun changeMiniThumbnail(command: ChangeActivityMiniThumbnailCommand): ChangeActivityResult {
        // id로 activity 있는지 확인
        ...
        val thumbnailInfo = uploadUrls.getOrThrow(FileCategory.MINI_THUMBNAIL)

        return ChangeActivityResult(thumbnailInfo.preSignedUrl, thumbnailInfo.fileKey)
    }

    @PreAuthorize("hasRole('ADMIN')")
    override fun completeThumbnailChange(command: CompleteActivityMiniThumbnailChangeCommand): ActivityResult {
        // 파일이 실제로 업로드 됐는지 확인
        ...
        val oldFileKey = activity.updateMiniThumbnailImage(newFileUrl, command.fileKey)
